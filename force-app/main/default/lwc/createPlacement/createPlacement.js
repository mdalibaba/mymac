...
import loadApplicationsGeo from '@salesforce/apex/CreatePlacementClass.loadApplicationswithGeo';
...
import getLatLngFromZipCode from '@salesforce/apex/CreatePlacementClass.getLatLngFromZipCode';
...
updatePaginatedData() {
    ...
    this.paginatedData = this.applications.slice(startIdx, endIdx);
    
    if(this.applications.length==1){
        this.paginatedData = this.applications;
    }
...
connectedCallback() {     
    ...
    /*if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                this.geoerror = 'Unable to retrieve location';
...
@wire(searchApplications, { name: '', address: '' ,position:'',zipcode:''})
...
handleSearch() {
    ...
    this.applications.forEach(element => {
        ...
        console.log( element.HRMSUS__Candidate1__r.rightpm__GeoLocations__latitude__s);
        console.log( element.HRMSUS__Candidate1__r.rightpm__GeoLocations__longitude__s);
       
        const latitude1 = element.HRMSUS__Candidate1__r.rightpm__GeoLocations__Latitude__s;
const longitude1 = element.HRMSUS__Candidate1__r.rightpm__GeoLocations__Longitude__s;
    ...
    this.selectedRadius='';
    searchApplications({ name: this.searchName, address: this.searchAddress ,position:this.position,zipcode:this.zipcode})
        .then(result => {
...
const longitude1 = element.HRMSUS__Candidate1__r.rightpm__GeoLocations__Longitude__s;
        ...
        console.log('distance--'+distance);
        if(rad==25){
        if(distance<=25 && distance<50 && this.locationAvailable){
            myarr.push(element);
        }
    }else if(rad==50){
            if(distance<=50 && distance>25 && distance<100 && this.locationAvailable){
                myarr.push(element);
            }
        }else if(rad==100){
            if(distance>=100 && distance>50 && distance<200 && this.locationAvailable){
                myarr.push(element);
            }
        }
        
        else if(rad==200){
            if(distance>=200 && distance>100 && distance<500 && this.locationAvailable){
                myarr.push(element);
            }
        }
        else if(rad==500){
            if(distance>=500 && distance>200 && this.locationAvailable){
                myarr.push(element);
            }
        }
    });
...
console.log(this.latitude+','+this.longitude,);
    this.applications.forEach(element => {
        ...
        console.log( element.HRMSUS__Candidate1__r.rightpm__GeoLocations__latitude__s);
        console.log( element.HRMSUS__Candidate1__r.rightpm__GeoLocations__longitude__s);
        lat=parseFloat(element.HRMSUS__Candidate1__r.rightpm__GeoLocations__latitude__s);
        long=parseFloat(element.HRMSUS__Candidate1__r.rightpm__GeoLocations__longitude__s);
        distance=this.calculateDistance(this.latitude,this.longitude,parseFloat(element.HRMSUS__Candidate1__r.rightpm__GeoLocations__latitude__s),parseFloat(element.HRMSUS__Candidate1__r.rightpm__GeoLocations__longitude__s));
          }});
