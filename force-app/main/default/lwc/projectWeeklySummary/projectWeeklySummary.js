...
let i=0;
export default class ProjectWeeklySummary extends NavigationMixin(LightningElement) {
		@track Payperiodlist = [];
		@track finalPrjAss = [];
										...
										if(ele.hasOwnProperty(day)){
												if(day=='Sun'){
														if(ename=='Over Time'){
																...
																ele.Sun.forEach(el1=>{
																	let myel1=el1.rightpm__OverTime__c;
																	OT_Total=OT_Total-myel1;
																	OT_Total=OT_Total+Number(evalue);
																	let rounded = Math.round(OT_Total * 100) / 100;
																	OT_Total=rounded;

																		el1.rightpm__OverTime__c=evalue;
																		if(evalue>0){
																			el1.rightpm__Per_Diem__c=perdiem ? true : el1.rightpm__Per_Diem__c;
																		}else if(evalue==0 && (el1.rightpm__StraightTime__c==''|| el1.rightpm__StraightTime__c==undefined) && perstatus=='Per Workday'){
